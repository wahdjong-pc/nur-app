...
// perintah hapus data berdasarkan id yang dikirimkan
  $query = $koneksi->query("DELETE FROM tbl_retribusi WHERE id_retribusi = '$id_retribusi'");
// cek perintah
  if ($query) {
    // pesan apabila hapus berhasil
    echo "<script>Swal.fire(
      'Terhapus!',
      'Data retribusi berhasil di hapus!',
      'success' 
    ).then((result) => {
    ...
    echo "<script>alert('Data retribusi gagal dihapus'); window.location.href='../data-retribusi.php'</script>";
  }
}
